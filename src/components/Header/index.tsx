...
const Header = (props: {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:8000/set-target-word", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("Failed to submit search query");
      }
            ...
            <div className="flex flex-row justify-between">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-transparent pl-9 pr-4 font-medium focus:outline-none xl:w-60"
                />
              </div>

              <button className={`hover:text-primary items-center ${isDefault ? "invisible" : "visible"}`} onClick={() => {}}>
